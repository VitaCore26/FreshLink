...
//
// Un cron Vercel quotidien appelle ?send=1 (voir vercel.json).
// ══════════════════════════════════════════════════════════════════════════════
...
export async function GET(req: NextRequest) {
  ...
  if (wantSend) {
    ...
    const res = await fetch(`${req.nextUrl.origin}/api/ext/send-email`, {
      method: "POST",
    ...
    return NextResponse.json({ ...rep, emailSent: res.ok, emailDetail: mail }, { headers: cors(origin) })
  }
